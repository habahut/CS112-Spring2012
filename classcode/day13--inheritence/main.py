...
class Explosion(Sprite):
    ...
    def rand_color(self):
        return randrange(120,255), 255, randrange(120,255)
...
class TieFighter(Ship):
    ...
    def update(self, dt):
        ...
        if (not (self.tx == self.vx)) or (not (self.ty == self.vy)):
            #print "bounce"
            pass
...
class YWing(Ship):
    ...
    def draw_image(self):
        ...
        self.orig_image = self.image
                                # flips the image over,     (in X),(not in Y)
...
class Game(Application):        # inherits applications methods
    title = "Spaceships"
    screen_size = 800, 600
    min_dt = 200
    max_ships = 600
    ...
    def __init__(self):
        ...
        self.ships = ShipGroup(self.max_ships)
                 ...
                 # ship type,    puts them in the ships Group
        self.spawners = [ TieSpawner(2000, self.ships, self.bounds),
                          YWingSpawner(2000, self.ships, self.bounds) ]

    def update(self):
        dt = min(self.clock.get_time(),self.min_dt)
                 # returns the miliseconds since last click,

        self.ships.update(dt)           # calls update on all the ships in the group
